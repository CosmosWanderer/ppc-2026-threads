...
bool ConvexHullOMP::RunImpl() {
  // Этап 1: Выделение компонент связности (последовательно, так как floodfill сложно распараллелить)
  ExtractConnectedComponents();

  // Этап 2: Параллельное построение выпуклых оболочек для каждой компоненты
  ExtractComponentsParallel();
...
bool ConvexHullOMP::PostProcessingImpl() {
  ...
  output = std::move(hulls_);
  return true;
}

void ConvexHullOMP::BinarizeImage(uint8_t threshold) {
#pragma omp parallel for
  for (int i = 0; i < static_cast<int>(working_image_.pixels.size()); ++i) {
...
void ConvexHullOMP::ExtractComponentsParallel() {
  size_t num_components = components_.size();
  hulls_.resize(num_components);

// Параллельное построение выпуклых оболочек для каждой компоненты
#pragma omp parallel for
  for (int i = 0; i < static_cast<int>(num_components); ++i) {
    hulls_[i] = ComputeConvexHull(components_[i]);
  }
}
